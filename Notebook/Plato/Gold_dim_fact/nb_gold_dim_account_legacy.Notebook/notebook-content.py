...
SILVER_TABLES = {"xero_accounts": "lh_bnj_silver.xero.silver_accounts"}
GOLD_DIMENSIONS = {"account": f"lh_bnj_gold.{tgt_catalog}.dim_account"}
GOLD_DIMENSIONS["account"]
...
start_job_instance(batch_id, job_id, msg="Start dim_inventory")
try:
    df = create_dim_inventory()
    src_cnt = spark.table(SILVER_TABLES["inventory"]).count()
    tgt_cnt = df.count()
    log_data_quality("dim_inventory", df, "inventory_key")
    df.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable(GOLD_DIMENSIONS["inventory"])
