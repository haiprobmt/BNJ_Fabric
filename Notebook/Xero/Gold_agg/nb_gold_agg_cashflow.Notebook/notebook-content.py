...
SILVER_SCHEMA_PLATO = f"`{WORKSPACE_NAME}`.lh_bnj_silver.plato"
SILVER_SCHEMA_XERO = f"`{WORKSPACE_NAME}`.lh_bnj_silver.xero"
GOLD_SCHEMA = f"`{WORKSPACE_NAME}`.lh_bnj_gold.{tgt_catalog}"


SILVER_TABLES = {"xero_invoices": f"{SILVER_SCHEMA_XERO}.silver_invoices"}
GOLD_FACTS = {"cashflow": f"{GOLD_SCHEMA}.fact_cashflow", "invoice": f"{GOLD_SCHEMA}.fact_invoice", "ar_aging": f"{GOLD_SCHEMA}.fact_ar_aging", "ap_aging": f"{GOLD_SCHEMA}.fact_ap_aging", "inventory": f"{GOLD_SCHEMA}.fact_inventory"}
AGG_TABLES = {"cashflow": f"{GOLD_SCHEMA}.agg_cashflow", "inventory": f"{GOLD_SCHEMA}.agg_inventory"}
GOLD_DIMENSIONS = {"date": f"{GOLD_SCHEMA}.dim_date"}
