...
def save_token_to_lakehouse(token_dict, file_path):
    ...
    # Save to lakehouse
    mssparkutils.fs.put(file_path, token_json, overwrite=True)
...
def load_token_from_lakehouse(file_path):
    ...
    try:
        # Read from lakehouse
        token_content = mssparkutils.fs.head(file_path, 10000)
...
#     # def get_tenant_id(self):
#     #     """Get tenant (organization) ID"""
#     #     df = spark.read.option("multiline", "true").json("abfss://c81c26a1-3386-4efb-b6ea-59363a6145b7@onelake.dfs.fabric.microsoft.com/c9d7507e-938a-4c6d-a042-d8743e386ab5/Files/xero/Token/xero_token.json")
#     #     access_token = df.select('access_token').collect()[0].access_token
...
#     #         print(f"Organization: {connections[0].get('tenantName', 'N/A')}")
#     #     else:
#     #         print("✗ No connections found. Check your access token.")
#     #     return self.tenant_id
#     def get_tenant_id(self):
#         """Get tenant (organization) ID"""
#         if self.tenant_id:
...
mssparkutils.notebook.exit("Authentication successful")
